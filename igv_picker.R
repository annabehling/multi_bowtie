...
igv_picker <- function(exp_file, readsum_file){
  ...
  red_readsum_tab <- readsum_tab[ ,c(1, 4, 2)] #give just the gene and parental columns (no +N) from the read summary table
  merged_tab <- merge(red_exp_tab, red_readsum_tab, by='GENE') #merge the reduced expression table with the read summary table
  merged_tab$p_reads <- merged_tab[ ,3] + merged_tab[ ,4] #make a new column with total reads mapped to both parents
  merged_tab$p1_prop <- (merged_tab[ ,3]/merged_tab$p_reads)*100 #make a new column with relative proportion of P1 reads
